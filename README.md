...
Учебный финансовый трекер на C++ и Qt Widgets. Проект специально сделан компактным: его реально разобрать за вечер и защитить как ООП-приложение.
...
## Как объяснять на зачете

1. Пользователь ставит лимиты по категориям.
2. Когда он добавляет расход, `FinanceManager::addExpense()` кладет объект `Expense` в историю и пересчитывает поле `spent` у нужной категории.
3. Остаток считается как `limit - spent`, поэтому при перерасходе он становится отрицательным.
4. Активы считаются отдельно: `Asset::marketValue()` возвращает `quantity * price`.
5. Вкладка инсайдеров показывает пример работы с сетью: `InsiderService` делает запрос, парсит таблицу и передает данные в `MainWindow` через сигнал.
